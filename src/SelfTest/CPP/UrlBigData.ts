...
inline void ADD(int o, int x, int y, int z)
{
    X[++ED] = x;
    Y[ED] = y;
    W[ED] = z;
...
int main()
{
    ...
    {
        scanf("%d%d", &n, &m);
        for (ed = ED = dfn = ans = i = 0; i <= n; i++)
            g[i] = f[i] = d[i] = size[i] = son[i] = top[i] = G[i] = bit[i] = 0;
        for (i = 1; i < n; i++)
        ...
        for (i = 1; i <= m; i++)
        {
            scanf("%d", &x);
            for (j = 0; j < 3; j++)
                scanf("%d%d", &y, &z), ADD(lca(x, y), x, y, z);
        }
        base = 1LL * m * VAL;
        go(1);
        if (ans / base == m)
...
export const CppUrlBigData = generateNormalSelfTest(
    "CppUrlBigData",
    "cpp",
    usrCode,
    {},
    ...
    undefined,
    {
        type: "url",
        hashsum:
            "c0342775c37ea6097d11e701bdfdf1a023698e4911b177002099186f120890aa",
        url: "https://www.ssst.top/bigdata.zip",
    }
);
